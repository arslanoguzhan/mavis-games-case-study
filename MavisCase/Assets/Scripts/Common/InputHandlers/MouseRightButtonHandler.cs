...
{
    public class MouseRightButtonHandler : ITickable
    {
        ...
        public void Tick()
        {
            ...
            else
            {
                ...
                {
                    (_downElement as IListener<MouseRightSwipeUp>)?.OnInteraction();
                    Clear();
                }
                else if(pos.y < _downPosition.y - _inputConfig.SwipeThreshold)
                {
                    (_downElement as IListener<MouseRightSwipeDown>)?.OnInteraction();
                    Clear();
                }
                else if(Input.GetMouseButtonUp(1) && duration <= _inputConfig.HoldDuration)
                {
                    (_downElement as IListener<MouseRightClick>)?.OnInteraction();
                    Clear();
                }
                else if(Input.GetMouseButtonUp(1) && duration > _inputConfig.HoldDuration)
                {
                    (_downElement as IListener<MouseRightHoldClick>)?.OnInteraction();
                    Clear();
                }
            }
        }
